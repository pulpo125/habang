...
if group_by_option == 'None':
    ...
    ## 데이터프레임 인덱스 rank로 변환
    tmp_df.index=tmp_df.index+1     # 인덱스 + 1 = rank
    ...
    tmp_df = df[group_by_option].value_counts()[:5].reset_index()       ## Top 5 추출
    tmp_df = pd.DataFrame(tmp_df).rename(columns={group_by_option:'count', 'index':group_by_option})
    st.bar_chart(tmp_df, x = group_by_option, y = 'count')  # 차트 생성
